...
/*
 * list of chunks that need other chunk to decode accurately:
 *  - IDAT (IHDR)
 *
 * not really important but good to have:
 *  - IHDR (need first IDAT to get compression level)
 * */
...
static void read_chunk(FILE *f)
{
	...
	while (not_iend) {
		...
		uint32_t check = crc32(0u, type, 4); // type
		// read chunk data
		printf("   Data = ");
		if (size > 0) {
			...
			check = crc32(check, data, size);  // data
		...
		// read chunk crc
		uint32_t chunk_crc;
		if (fread(&chunk_crc, sizeof(uint32_t), 1, f) != 1) {
